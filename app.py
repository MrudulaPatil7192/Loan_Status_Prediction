        ...
        <form method="POST" action="/predict">
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="number" id="no_of_dependents" name="no_of_dependents" min="0" required placeholder="e.g. 2">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="income_annum" name="income_annum" step="any" required placeholder="e.g. 5000000">
                </div>

                <div class="form-group">
                    <label for="loan_amount">Loan Amount ($)</label>
                    <input type="number" id="loan_amount" name="loan_amount" step="any" required placeholder="e.g. 15000000">
                </div>

                <div class="form-group">
                    <label for="loan_term">Loan Term (Years)</label>
                    <input type="number" id="loan_term" name="loan_term" min="1" required placeholder="e.g. 10">
                </div>

                <div class="form-group">
                    <label for="cibil_score">CIBIL Score</label>
                    <input type="number" id="cibil_score" name="cibil_score" min="300" max="900" required placeholder="e.g. 750">
                </div>

                <div class="form-group">
                    <label for="residential_assets_value">Residential Assets ($)</label>
                    <input type="number" id="residential_assets_value" name="residential_assets_value" step="any" required placeholder="e.g. 2000000">
                </div>

                <div class="form-group">
                    <label for="commercial_assets_value">Commercial Assets ($)</label>
                    <input type="number" id="commercial_assets_value" name="commercial_assets_value" step="any" required placeholder="e.g. 4000000">
                </div>

                <div class="form-group">
                    <label for="luxury_assets_value">Luxury Assets ($)</label>
                    <input type="number" id="luxury_assets_value" name="luxury_assets_value" step="any" required placeholder="e.g. 8000000">
                </div>

                <div class="form-group">
                    <label for="bank_asset_value">Bank Assets ($)</label>
                    <input type="number" id="bank_asset_value" name="bank_asset_value" step="any" required placeholder="e.g. 3000000">
                </div>
...
def predict():
    if model is None:
        return render_template_string(
            HTML_TEMPLATE, 
            prediction_text="Error: Model file 'xgboost_model.pkl' not found.", 
            result_class="rejected"
        )

    try:
        # Extract features in exact order as expected by XGBoost
        features = [
